...
## Website & Demo Architecture (2026 Hackathon)

**CommunityOne’s public demo is a static web app — no backend, no login, just data.**

- **Live demo:** [https://getcommunityone.github.io/c1_gemma_4_good/](https://getcommunityone.github.io/c1_gemma_4_good/)
- **Colab pipeline:** [run_in_colab.ipynb](https://colab.research.google.com/github/getcommunityone/c1_gemma_4_good/blob/main/scripts/colab/run_in_colab.ipynb)
- **Published dataset:** [Hugging Face: CommunityOne/one-jurisdiction-mapping-analysis](https://huggingface.co/datasets/CommunityOne/one-jurisdiction-mapping-analysis)

### How it works

1. **Run the pipeline** (Colab or local):
  - Ingests municipal PDFs, audio, and images.
  - Outputs structured JSON and Markdown summaries.
2. **Export for web:**
  - `python scripts/colab/export/export_web_demo_index.py` creates meeting/search JSON for the website.
3. **Static web UI:**
  - React + Vite + TailwindCSS, deployed to GitHub Pages.
  - No backend — all data is loaded from static files in `web/public/data/`.
  - Features: Meeting explorer, full drilldown, ACS data explorer, search, and more.

#### Data Flow Diagram

```mermaid
flowchart LR
  A[Colab pipeline\n(run_in_colab.ipynb)] --> B[Processed outputs\n(JSON, Markdown)]
  B --> C[Export script\n(export_web_demo_index.py)]
  C --> D[Static web data\n(web/public/data/…)]
  D --> E[Web UI\n(GitHub Pages)]
  E -->|Live| F([User browser])
```

**All meeting summaries, categories, and search data are generated by the pipeline and published as static files.**
