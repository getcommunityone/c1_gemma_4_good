...
export default function DataExplorerScorecardPage() {
  ...
  const {
    ...
    queryFn: async (): Promise<CountyPlaceTrendsSidecar | null> => {
      ...
      const r = await fetch(dataUrl(`data/census-map/county_trends_${locationFips}.json`))
  ...
  const {
    ...
    queryFn: async (): Promise<CountyPlaceTrendsSidecar | null> => {
      if (!locationFips) return null
      const r = await fetch(dataUrl(`data/census-map/place_trends_${locationFips}.json`))
      if (r.status === 404) return null
